...
> 🔔 **已經建好這個動作的人**：`feature_row` 是 2026-09-18 才加的一行，請回去補上（連同 `"prediction"` 那行結尾的逗號）。缺了它，這筆評估**日後無法產出付費報告**——購買端點會擋下來並回 409，而且**送件當下看不出任何異常**：免費信照常寄出，要等到有人付款才會發現。
>
> ⚠️ **上面的 `HTTP` 是動作名稱，不是固定字串。** 把 `/predict` 那個動作改名（例如 `HTTP_AI_predict`，比預設的 `HTTP` / `HTTP 1` 清楚得多）之後，本文裡每一處 `body('HTTP')` 都要跟著改成新名字。`剖析_JSON` 同理。
>
> 補完之後**務必實際送一筆並查回來確認**——本文裡有那行、跟資料庫真的收到值，是兩件事。查法見第 6 步。
...
#### ⚠️ 已知不一致：信件的分級系統性地比模型嚴重

信件樣板**沒有**使用 API 的 `final_risk_level`，而是自己拿 `risk_score` 比 `0.25` / `0.5` 算出「低／中度／較高相對風險」。

以 11 組合成檔案實測（`risk_score` 涵蓋 0.386～0.900）：

| risk_score | API 分級 | 信件顯示 | |
|---|---|---|---|
| 0.386 | 低風險 | **中度**相對風險 | ✗ |
| 0.500 | 中度風險 | **較高**相對風險 | ✗ |
| 0.556 | 中度風險 | **較高**相對風險 | ✗ |
| 0.626 ～ 0.900 | 高風險 | 較高相對風險 | ✓ |

**4 / 11 不一致，而且全部同一個方向：信件講得比模型嚴重。**

推得的門檻：

```
API  ：低 < ~0.40 ≤ 中度 < ~0.60 ≤ 高
信件 ：低 < 0.25  ≤ 中度 < 0.50  ≤ 較高
```

因此有兩段會誤報：

- **`0.25 ～ 0.40`** — 信件說「中度」，模型說「低」
- **`0.50 ～ 0.60`** — 信件說「較高」，模型說「中度」

#### 同一封信自己前後矛盾

信件底部的驗證摘要寫著：

> 以上為內部研究資料在 **Pan-Cancer 判定閾值 0.40** 下的平均驗證表現

但同一封信頂部的分級用的是 `0.50`。**信件自己宣告模型的門檻是 0.40，卻拿 0.50 去分級。** 而 0.40 正好與實測推得的 API 低／中度界線吻合。

#### 為什麼這件事重要

方向是最糟的那一邊：**告訴人家風險比模型實際判定的更高**。在癌症風險產品上這會造成不必要的驚嚇，而且被質疑時沒有依據可辯護——信件引用的權威門檻就印在它自己的頁尾。

這是現行線上就存在的問題，與免費版無關，但免費版會把它擴散到一般大眾。**上線前必須決定以哪一邊為準**；這是臨床／法規判斷，不是工程選擇。

在決定之前，`risk_band` 存 API 的 `final_risk_level`（模型端的權威值）。這代表**資料庫存的分級會與使用者收到的信不一致**，直到上述決定落實為止。
...
| **付款區塊** | **新增**，就接在移除的位置 |
| `/predict` 動作的名稱 | `body('HTTP')` → `body('HTTP_AI_predict')`，中文 7 處、英文 6 處 |
| 其餘全部（標題、風險指數、分級、等級說明、各癌別完整排序、個人化建議、免責聲明） | **與付費版逐位元組相同** |

### 5.2b `/predict` 動作叫什麼名字

流程 A 把那個動作叫 `HTTP`，流程 B 叫 `HTTP AI predict`（運算式裡寫成 `HTTP_AI_predict`，空格變底線）。樣板產生時就已經改好，直接貼即可。

貼上時若出現「**動作 '傳送電子郵件_(V2)_1' 的輸入參數包含 'HTTP' 的無效參考**」，代表樣板裡的名字跟流程 B 裡的對不上。**不要在 HTML 檢視裡手動取代** —— 那是 13 處，漏一處就會在寄信當下才失敗。改 `scripts/build-email-templates.js` 裡的 `PREDICT_ACTION` 常數，重新產出。
...
| **信件分級系統性高於模型分級** | 兩段區間（0.25~0.40、0.50~0.60）信件講得比模型嚴重;信件頁尾自述門檻 0.40，卻用 0.50 分級 | **上線前必須決定以哪一邊為準**（臨床／法規判斷），見 3.4 |
