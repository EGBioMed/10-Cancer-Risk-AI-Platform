...
if ( ! function_exists( 'egbio_handle_recommendation_link' ) ) {
	function egbio_handle_recommendation_link() {
		...
		// 這個回應不可被快取。信件裡的連結每個癌別只有一個固定網址，所以
		// 一旦被存進快取，改對應表之後客戶仍會被送到舊產品——而且從程式碼
		// 完全看不出來。2026-09-21 部署當天就踩到了：部署前測試留下的 404
		// 被快取，部署後同一個網址繼續回 404，其他癌別卻正常。
		//
		// 這一行擋不住已經存在的快取條目（頁面快取在 PHP 之前就回應了），
		// 所以 W3 Total Cache 的「永不快取的頁面」也要加上 /recommend。
		// 兩層都要：設定會被人改掉，程式碼不會。
		nocache_headers();
