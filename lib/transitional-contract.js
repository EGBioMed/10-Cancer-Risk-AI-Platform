...
const ROOT_FIELDS = new Set([
  ...
  "submitted_at", "full_name", "email", "language", "report_language", "consent_record",
...
// 必須與 app.js 的同名常數一致（test-contract.js 會比對兩份）。這些欄位不是模型特徵，
// 但必須待在 ai_api_feature_row 裡，因為 Power Automate 的 HTTP action 把該物件原封不動
// 當 API body 送出。少了這個豁免，71 欄的 columns 對不上 72 個鍵，整批送件會被判
// row_shape_mismatch——2026-09-07 前端與後端就是同時壞在這一點上。
const AI_API_REPORT_ONLY_FIELDS = ["country", "symptoms"];

// 允許但不強制：舊分頁還握著上一版 app.js 時，強制會讓那些送件全數被拒，
// 而缺 country 只會讓報告退回台灣基準並在報告上明寫分母，缺 symptoms 只會讓規則層
// 回到 2026-09-08 之前的狀態（一律印「未觸發」），兩者都比擋掉送件溫和得多。
function sameKeysAllowingExtras(actual, expected, allowedExtras) {
  if (!isPlainObject(actual)) return false;
  const keys = Object.keys(actual);
  return expected.every((key) => keys.includes(key))
    && keys.every((key) => expected.includes(key) || allowedExtras.includes(key));
}
...
function validateTransitionalSubmission(submission) {
  ...
  if (typeof submission.full_name !== "string"
      || !submission.full_name.trim()
      || submission.full_name.length > 100
      || /[<>\u0000-\u001f\u007f]/u.test(submission.full_name)) {
    add("$.full_name", "invalid_name", "A valid participant name is required.");
  }
  ...
  for (const [columnsName, rowName] of VECTOR_DEFINITIONS) {
    ...
    if (!sameKeysAllowingExtras(submission[rowName], expected, allowedExtras)) {
      add(`$.${rowName}`, "row_shape_mismatch", allowedExtras.length
        ? `Row keys must match ${columnsName}, optionally plus ${allowedExtras.join(", ")}.`
        : `Row keys must exactly match ${columnsName}.`);
    }
  }
  ...
  if (!Array.isArray(submission.rows) || submission.rows.length < 1 || submission.rows.length > 100) {
    ...
    submission.rows.forEach((row, index) => {
      ...
      if (row.full_name != null || row.question_id === "full_name") add(`$.rows[${index}].full_name`, "identifier_leak", "Participant name is not allowed in answer rows.");
  ...
  if (!isPlainObject(submission.excel_row)) {
    ...
    if ("full_name" in submission.excel_row) add("$.excel_row.full_name", "identifier_leak", "Participant name is not allowed in the research row.");
  ...
  if (!isPlainObject(submission.contact_row)) {
    ...
    const contactKeys = ["record_id", "full_name", "email", "submitted_at", "language", "report_language"];
    ...
    if (submission.contact_row.full_name !== submission.full_name) add("$.contact_row.full_name", "value_mismatch", "Contact name must match the participant name.");
...
// Compatibility boundary for browser tabs that were opened before direct
// contact fields were split from research answers. Keep the strict validator
// strict, but remove legacy duplicate identifiers before validating or
// persisting an HTTP submission. The canonical root/contact values remain.
function sanitizeLegacyDirectIdentifiers(submission) {
  if (!isPlainObject(submission)) return submission;

  const sanitized = { ...submission };
  if (Array.isArray(submission.rows)) {
    sanitized.rows = submission.rows
      .filter((row) => isPlainObject(row) && !["full_name", "email"].includes(row.question_id))
      .map((row) => {
  ...
  if (isPlainObject(submission.excel_row)) {
    sanitized.excel_row = { ...submission.excel_row };
    delete sanitized.excel_row.full_name;
    delete sanitized.excel_row.email;
  }
  return sanitized;
}
