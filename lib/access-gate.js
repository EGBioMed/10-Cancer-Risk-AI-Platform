...
// Signature check and payload decode, WITHOUT the expiry check.
//
// Split out so that a session which lapsed can be told apart from no
// session at all. Someone who spent twenty minutes answering and then hit
// the gate on submit deserves to be told what actually happened rather than
// a reference code -- but only a cookie this server signed may be reported
// that way, which is why the signature still has to verify before the
// expiry is even read.
function readSignedSessionPayload(cookieValue, secret) {
...
function verifySessionCookie(cookieValue, secret, now) {
  const payload = readSignedSessionPayload(cookieValue, secret);
...
// Forces the browser to drop the session cookie immediately (Max-Age=0),
// instead of it lingering for the rest of ACCESS_GATE_SESSION_TTL_HOURS.
// Used right after a report submission succeeds -- pairs with
// createConsumedSessionStore below, which is the server-side half of the
// same rule (the cookie is still cryptographically valid until its own
// exp, so clearing it client-side alone wouldn't stop a replayed copy).
function buildExpiredCookieHeader(name, { secure = true } = {}) {
  const parts = [
    `${name}=`,
    "Path=/",
    "HttpOnly",
...
// Marks a session (by its per-redemption `sid`, not by grantId -- a shared
// institution code hands out one grantId to many independent sessions, and
// only the one that just submitted should be cut off) as having completed
// one report submission, so a second /api/submit replaying the same
// still-unexpired cookie is rejected -- each submission then requires its
// own fresh code/link entry, matching the per-person quota model instead of
// "per browser session, however many submissions fit in the 30-minute TTL."
// In-memory and per-instance, same tradeoff as lib/rate-limiter.js's
// limiters: a process restart forgets consumed sessions, which merely
// re-opens a session that was already going to expire soon on its own --
// it never grants anything new.
function createConsumedSessionStore() {
  const consumedUntil = new Map();
  return {
    consume(sid, expiresAtSeconds) {
      if (sid) consumedUntil.set(sid, expiresAtSeconds || 0);
    },
    isConsumed(sid) {
      return Boolean(sid) && consumedUntil.has(sid);
    },
    sweep(now = Math.floor(Date.now() / 1000)) {
      for (const [sid, exp] of consumedUntil) {
        if (exp <= now) consumedUntil.delete(sid);
      }
    }
  };
}
