...
function buildResearchRecord(submission) {
  ...
  if (record.excel_row && typeof record.excel_row === "object") {
    delete record.excel_row.full_name;
...
function createPostgresRepository(options = {}) {
      ...
      if (!existingContact.rowCount) {
        await client.query(`
          INSERT INTO contact.delivery_contacts (
            record_id, full_name, email, submitted_at, language, report_language, payload_hash
  ...
  async function createAccessGrant({
    ...
    try {
      ...
      `, [tokenHash, paymentProvider, paymentReference, maxUses, expiresAt, createdBy, notes, credentialType, code]);
      ...
      `, [grant.grant_id, json({ paymentProvider, createdBy, credentialType })]);
      await client.query("COMMIT");
  ...
  async function redeemAccessGrant({ tokenHash }) {
    ...
    try {
      ...
      // expires_at is NULL for quota-only codes (no time limit). Written as
      // an explicit IS NOT NULL check rather than relying on
      // "NULL < clock_timestamp()" evaluating to SQL NULL (falsy in JS).
      const expiryCheck = await client.query(
        "SELECT $1::timestamptz IS NOT NULL AND $1::timestamptz < clock_timestamp() AS expired",
        [grant.expires_at]
      );
  ...
  async function getAccessGrantStatus({ tokenHash }) {
    const result = await pool.query(`
      SELECT grant_id, credential_type, code, status, payment_provider, payment_reference,
             max_uses, use_count, issued_at, expires_at, redeemed_at, revoked_at, created_by, notes
      FROM access.grants
      WHERE token_hash = $1
    `, [tokenHash]);
  ...
  async function topUpAccessCode({ tokenHash, addUses, actor }) {
    ...
    try {
      await client.query("BEGIN");
      const existing = await client.query(`
        SELECT grant_id, status, max_uses, use_count, credential_type
        FROM access.grants
        WHERE token_hash = $1
        FOR UPDATE
      `, [tokenHash]);
      ...
      const newMaxUses = grant.max_uses + addUses;
      await client.query(`
        UPDATE access.grants
        SET max_uses = $2,
            status = CASE WHEN use_count < $2 THEN 'issued' ELSE status END
        WHERE grant_id = $1
      `, [grant.grant_id, newMaxUses]);
      await client.query(`
        INSERT INTO operations.access_events (grant_id, event_type, event_status, detail)
        VALUES ($1, 'grant_topped_up', 'success', $2)
      `, [grant.grant_id, json({ addUses, newMaxUses, actor })]);
      await client.query("COMMIT");
      return { ok: true, grantId: grant.grant_id, maxUses: newMaxUses, useCount: grant.use_count };
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
  ...
  async function listContactRecords() {
    const result = await pool.query(`
      SELECT record_id, full_name, email, submitted_at, language, report_language, delivery_status
