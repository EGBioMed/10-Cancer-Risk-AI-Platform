...
|---|---|
| 版本 | freemium/1.0.0 |
| 狀態 | **技術上已可運作**——第 1～11 步全部完成並實測通過，整條鏈路（免費信 → 付款驗票 → 產報告 → 附件寄達）於 2026-09-18 走通。剩下 WooCommerce 上架、IRB 補件、公布代碼三項，見第 10 節 |
...
| 10 | 由流程 **A** 複製出流程 C（報告交付） | 實際交付一份 PDF，內容與免費信一致 | ✅ 2026-09-18 實測通過 |
...
2. **流程 A 自第 5 步起未被編輯過一次**;機構線的 payload 也逐欄不變（§7.6）。免費線與交付線的每一次測試,流程 A 的執行紀錄都沒有新增。
3. **在第 14 步公布代碼之前,免費線只有一個入口**(測試碼 `egbiotest2026`),所以前 13 步都在正式環境安全完成。

回退動作依嚴重程度遞增:撤銷宣傳代碼 → 清空 `POWER_AUTOMATE_WEBHOOK_URL_PUBLIC`(免費線 503 不寄信) → 清空 `REPORT_DELIVERY_FLOW_URL`(付款仍記錄,交付標記失敗)。**三者都不需要碰流程 A。**

### 10.1 2026-09-18 實測通過的整條鏈路

```
公開代碼 egbiotest2026 → 問卷 → 流程 B → 免費信（無附件、有付款連結）
                                       → report_results 存下輸出與輸入
簽章票券 → /api/reports/purchase → 驗票、查存檔、記錄付款 → 流程 C
                                → 讀存檔 → 產報告 → 轉檔 → 附件寄達
```

四項驗收皆過:交付信是交付信（非報告複本）、PDF 分數與免費信一致、流程 A 與 B 皆無新執行、重送同一 `order_reference` 回 `reused:true` 且不重複寄送。

### 10.1a 2026-09-18 真實付款實測

上面那條鏈路當時是以人工簽發的票券驗證的,商店那一段是補上的最後一環。同日以**真實付款**走完全程,商品 ID 1062(SKU `AI-CANCER-REPORT`,隱藏不上架):

```
egbiotest2026 → 問卷 → 免費信 → 點「取得完整報告」→ 加入購物車 → 結帳付款
              → plugin 轉交票券 → /api/reports/purchase → 流程 C → PDF 寄達
```

評估 `d553492a-ff72-4ca7-8552-72dc563d3355`,免費信與 PDF 同為 64.00 分／高風險／大腸直腸癌。

途中發現並修掉的三件事:

1. **無票券不得購買**這道防線實測有效(直接開 `?add-to-cart=1062` 購物車維持空的)。但通知只在購物車／結帳頁印出,首頁看不到——排查時要去 `/cart/` 看。
2. **接票券的 hook 與 WooCommerce 撞優先序**,兩者都掛 `wp_loaded` 20,誰先跑取決於外掛載入順序。已降為 5(`283b56e`)。
3. **免費信樣板引用的動作名稱**是流程 A 的 `HTTP`,流程 B 叫 `HTTP AI predict`,貼上時被拒。改名已移入產生器的 `PREDICT_ACTION` 常數(`26d4852`)。

另外記一筆事故:當天 07:21 UTC,以 WordPress 外掛編輯器貼上擴充內容時發生 fatal,編輯器的自動還原失敗,把 `egbio-access-code.php` 留成 **0 bytes**,存取代碼交付停擺約 30 分鐘且無任何訂單註記可循。**此後外掛一律不得經由外掛編輯器修改**,改走上傳 → `php -l` → `cp` 的路徑(容器內有 PHP CLI,位於 `/usr/local/bin/php`)。

### 10.2 上線前仍待處理

| 項目 | 性質 |
|---|---|
| ~~流程 C 的一致性檢查~~ | ✅ 2026-09-21 建好並雙向驗證（見 §10.4） |
| ~~訂單註記的假警報~~ | ✅ `37fc8b2`,已部署 |
| 流程 A 的分級 bug 仍在線上 | 影響**現有付費客戶**,與本專案無關但同源（見 `FLOW_B_RUNBOOK.md` 3.4） |
| 撤銷測試碼 `egbiotest2026` | 公布正式宣傳代碼前 |
...
| IRB 補件 | **硬門檻**,擋在 §10.3 那 13 組代碼對外發放之前 |

### 10.3 免費版廠商代碼

免費線的代碼格式與機構碼**不同**,規則是 `EG` + 廠商縮寫 + 3 個亂碼,每組 `--max-uses 1000`、`--delivery-mode public`。

機構碼的格式是 `<機構><Q><額度><6 亂碼>`(例如 `grmnq59kxmfp`),額度直接寫在代碼裡。免費版這個格式**刻意不帶額度**,換取一個短到能印在推廣文宣上的字串。代價是客服無法從代碼本身看出剩餘額度,只能查:

```bash
npm run access:status -- --code <code>
```
...
三碼亂碼約 29,791 種組合。推廣碼本來就要公開發放,被猜中不算外洩,但猜中的人會吃掉額度並歸到該廠商名下,正好破壞「分辨流向」這個目的。若日後歸因準確度變重要,改用四碼(約 92 萬組)。額度用盡可用 `scripts/topup-access-code.js` 加,不必重鑄。

### 10.4 流程 C 一致性檢查(2026-09-21 完成)

實作見 `FLOW_C_RUNBOOK.md` §5。比對的是**免費信承諾過的三個數字**:風險指數、風險分級、第一名癌別。任何一項不符即終止,不寄信。

設計上最關鍵的一點是**比較的是客戶看到的字串,不是浮點原始值**。實測證據:同一筆評估,重新評分回 `risk_score_pct = 64`(整數),資料庫存的是 `DECIMAL(6,2)` 的 `"64.00"`。原先規劃的直接比對在 Power Automate 裡會因型別不同而恆不相等,結果是**每一筆交付都被擋掉**——一個看起來像「模型變了」的全面停擺。兩邊先 `formatNumber(...,'0.0')` 化為 `"64.0"` 再比,才對得起這道檢查存在的理由。

雙向驗證(record `d553492a`):

| 測試 | 做法 | 結果 |
|---|---|---|
| 會擋 | 暫時把分級那列右值改成寫死的錯字串 | 流程 C `Failed`,終止訊息印出六個值,**未寄出任何信** |
| 會放行 | 改回正確運算式 | 流程 C `Succeeded`,收到含 PDF 的信,64.0／高風險／大腸直腸癌 |

> 建好後**必須測「會擋」那一側**。一個永遠回傳 true 的檢查與沒有檢查等價,而且更糟——它會讓人以為有保護。
